-- Active: 1758675041396@@127.0.0.1@3306@quiz
-- 1. 집계 함수 확인 문제
...
-- 정답:
SELECT SUM(quantity) FROM orders WHERE price >= 1000000;
...
-- 정답:
SELECT SUM(quantity * price) FROM orders WHERE price <= 1000000 AND quantity >= 2;
...
-- 정답:
SELECT COUNT(*) FROM orders WHERE order_date BETWEEN  '2023-11-01' AND  '2023-11-05';
...
-- 정답:
SELECT DISTINCT * FROM orders WHERE region = '대구' OR customer_name = '김철수';

-- 문제 7
-- '스마트폰'이 총 몇 대 팔렸는지 조회하세요.

-- 정답:
SELECT COUNT(quantity) FROM orders WHERE product = '스마트폰';
