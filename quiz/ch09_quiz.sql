-- Active: 1758675041396@@127.0.0.1@3306@company
-- 1. 서브쿼리 확인 문제
...
SELECT SUM(project_id) FROM employee_projects GROUP BY employee_id;
...
SELECT 
	...
	sr.amount 
FROM employees e
JOIN salary_records sr 
	ON sr.employee_id = e.id
ORDER BY sr.amount DESC LIMIT 1
...
WHERE sub.ma = (SELECT MAX(amount) FROM salary_records)

-- 문제 5: HAVING 절에서의 서브쿼리
-- 부서별 평균 급여가 전체 평균 급여 이상인 부서명을 조회하세요.

-- 정답:
SELECT dp.name,AVG(salary)
FROM departments dp
JOIN employees e ON e.department_id = dp.id
GROUP BY dp.id
...
ORDER BY COUNT(ep.project_id) DESC LIMIT 1;


SELECT p.name,sub.c
FROM projects p
JOIN (
	SELECT project_id, COUNT(project_id) as c
	FROM employee_projects
	GROUP BY project_id
) as sub ON sub.project_id = p.id
ORDER BY sub.c DESC LIMIT 1;
