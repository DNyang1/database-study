-- Active: 1758675041396@@127.0.0.1@3306@mapdonalds
-- 주석
-- 쿼리 설명을 위한 코드(실행되지않음)
# 또 다른 한줄 주석
/*  
...
/*
  (참고) SQL에 제공하는 기본 키워드들은 대소문자를 구분하지않음
  권장 SQL 작성법은 일반적인 관리에 따라 
  대문자: MySQL 키워드(SELECT, FROM, WHERE, JOIN 등)
  소문자: 테이블명, 컬럼명, 별칭, 기타 사용자 정의 식
...
/*
  ...
  2.1 데이터 CRUD
*/
-- CRUD란? DB가 제공하는 가장 기본 동작으로 데이터 관리의 가장 핵심적인 기능
-- 생성
-- 조회
-- 수정
-- 삭제
...
-- 실무 Tip!
-- 데이터베이스 이름은 보통 프로젝트 이름이나 서비스 특징을 나타내는 것으로 생성
-- 소문자와 언더스코어(_)를 조합하여 만드는 것이 일반적인 관례(ex: my_shop)
...
CREATE TABLE burgers (
  id INTEGER, -- 아이디(정수)
  -- id INT PRIMARY KEY, 또는 이렇게 써도 됨
  name VARCHAR(50), -- 이름(최대 50글자의 문자)
  -- VARCHAR(N): 문자열 최대 N자리까지 저장(가변 길이 타입이라저장될 수 있는 최대 길이의 제한을 알려줘야함)
  price INTEGER, -- 가격(정수)
  gram INTEGER, -- 무게(정수)
  kcal INTEGER, -- 칼로리(정수)
...
-- 예: 포인트 제도가 새로 생겨서 고객 테이블에 point 열을 추가해야 할 떄
