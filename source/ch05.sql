-- Active: 1758675041396@@127.0.0.1@3306@data_type
/*
	5. 다양한 자료형 활용하기
	5.1 자료형이란
*/
...
-- UNSIGNED(언사인드) 제약조건 부여 가능: 음수 값을 허용하지 않는 정수
-- 유효성 보장: 0~255 의 유효한 값만 저장
CREATE TABLE users (
...
CREATE Table student_records (
    id INTEGER, -- 아이디 (모든 정수)
    grade TINYINT UNSIGNED, -- 학년(부호가 없는 매우 작은 정수), 0~255
    average_score FLOAT, -- 평균 점수 (부동 소수점 방식의 실수)
    tuition_fee DECIMAL(10,2), -- 수업료 (고정 소수점 방식의 실수), 돈계산 관련은 정확하게 전체 10, 소수점 2
    PRIMARY KEY (id)
);
INSERT INTO student_records (id, grade, average_score, tuition_fee) VALUES
    (1, 3, 88.75, 50000.00),
    (2, 6, 92.5, 100000.00);
...
-- CHAR와 VARCHAR 자료형 사용 예
CREATE TABLE addresses (
    postal_code CHAR(5), -- 우편번호 (고정길이 5자)
...
-- 과거에는 "길이가 고정적이면 CHAR, 가변길이면 VARCHAR" 라는 원칙이 강조되었지만
-- 지금은 그냥 VARCHAR 쓴다
...
-- 그 경로나 URL만 데이터베이스에 저장하는 방식이 표준

-- 왜 DB에 저장하지 않는가
-- DB 부하가 큼, 성능이 느림, 백업 복구 어려움
